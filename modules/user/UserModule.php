...
class UserModule extends CWebModule {
    ...
    public static function getAdmins() {
        if (!self::$_admins) {
            $admins = User::model()->superuser()->findAll();
            $return_name = array();
            foreach ($admins as $admin)
                array_push($return_name, $admin->display_name);
    ...
    public static function user($id = 0) {
        if ($id)
            return User::model()->find("id_user=:id_user", array(":id_user" => $id));
        else {
            if (Yii::app()->user->isGuest) {
                return false;
            } else {
                if (!self::$_user)
                    self::$_user = User::model()->find("id_user=:id_user", array(":id_user" => Yii::app()->user->id));
                return self::$_user;
            }
        }
    }
