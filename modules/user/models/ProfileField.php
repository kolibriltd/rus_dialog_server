...
class ProfileField extends CActiveRecord
{
	...
	const REQUIRED_YES_NOT_SHOW_REG = 3;
        ...
        const VISIBLE_USER_ALL=0;
	const VISIBLE_CUSTOMER_USER=2;
	const VISIBLE_CONTRACTOR_USER=3;
	...
	public function rules()
	{
		...
		return array(
			...
			array('field_size, field_size_min, required, position, visible, user_visible', 'numerical', 'integerOnly'=>true),
	...
	public function attributeLabels()
	{
		return array(
			...
			'visible' => UserModule::t('Visible'),
                        'user_visible'=> UserModule::t('Права на заполнение'),
		);
	}
		...
		$_items = array(
			...
			'visible' => array(
				...
				self::VISIBLE_NO => UserModule::t('Hidden'),
                        ...
                        'user_visible' => array(
				self::VISIBLE_USER_ALL => UserModule::t('Все'),
				self::VISIBLE_CUSTOMER_USER => UserModule::t('Заказчики'),
				self::VISIBLE_CONTRACTOR_USER => UserModule::t('Подрядчики'),
                            ),
		);
		if (isset($code))
